...
import {
  CalendarDays, Siren, Truck, Boxes, GraduationCap, Users,
  Flame, ExternalLink, FolderOpen, Instagram, ArrowLeft,
} from "lucide-react";
import brasao from "@/assets/sobre/brasao-3gb.png.asset.json";

const URL_PE_SEGURO = "AJUSTAR";

type PortalLink = { titulo: string; descricao?: string; url: string; interno?: boolean; };
type Secao = { titulo: string; Icone: typeof Flame; links: PortalLink[]; };
...
const SECOES: Secao[] = [
  ...
  },
  {
    ...
    Icone: Users,
    links: [
      { titulo: "Pilotos de Drone", url: "https://docs.google.com/spreadsheets/d/1I1WELedaFKXuK9KxcOEab6uPl1idylVSJECAUXhGzio/edit?usp=sharing" },
      { titulo: "Contatos de Municípios — 3º GB", url: "https://docs.google.com/spreadsheets/d/1Qe5DQ14TpiXRZgzF-WQGsVClQGw4eNKpvFEId2kent4/edit?usp=sharing" },
      { titulo: "Instagram @3gbcbmpe", url: "https://www.instagram.com/3gbcbmpe" },
    ],
  },
];
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Acesso rápido aos sistemas, escalas, formulários e contatos do serviço operacional do 3º Grupamento de Bombeiros." },
      { property: "og:title", content: "Portal Operacional — 3º GB" },
      { property: "og:description", content: "Sistemas e documentos do serviço operacional do 3º GB em um só lugar." },
    ],
  }),
    ...
    <div className="min-h-screen">
      <header className="hero-fire relative overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-5xl px-4 py-8 text-center md:py-12">
          <img src={brasao.url} alt="Brasão 3º GB" className="float-3d brasao-fire mx-auto mb-4 h-20 w-20 object-contain md:h-24 md:w-24" />
          ...
          <h1 className="title-fire text-2xl font-bold tracking-tight md:text-4xl">Portal Operacional</h1>
          <p className="mt-2 text-sm text-white/70 md:text-base">3º Grupamento de Bombeiros — CBMPE</p>
        </div>
      ...
      <main className="mx-auto max-w-5xl px-4 py-8 md:py-10">
            ...
            <section key={secao.titulo}>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {secao.links.map((link) => {
                  ...
                  const conteudo = (
                    <>
                      <div className="min-w-0 flex-1">
                        <p className="truncate font-medium text-foreground">{link.titulo}</p>
                        {link.descricao && <p className="mt-0.5 truncate text-xs text-muted-foreground">{link.descricao}</p>}
                      </div>
                  ...
                  return link.interno ? (
                    <Link key={link.titulo} to={link.url as any} className={classes}>{conteudo}</Link>
                  ) : (
                    <a key={link.titulo} href={link.url} target="_blank" rel="noopener noreferrer" className={classes}>{conteudo}</a>
                  );
                })}
              </div>
            </section>
        ...
        <footer className="mt-12 border-t border-border pt-6 text-center">
          <p className="text-xs uppercase tracking-[0.2em] text-white/50">Honra · Disciplina · Coragem</p>
          <div className="mt-4 flex items-center justify-center gap-6 text-xs">
            <Link to="/sobre" className="text-muted-foreground transition-colors hover:text-primary">Sobre o 3º GB</Link>
          </div>
        </footer>
      </main>
    </div>
